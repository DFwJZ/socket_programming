...
async fn handle_control_channel(mut control_stream: TlsStream<TokioTcpStream>) {
    ...
    if (username == "1" && password == "2") || (username == "3" && password == "4") {
        // Generate uniqe token, could use uuid crate
        // let token = format!("{}:{}", username, password);

        // Send success message to client to allow connection
        let success = "Authentication successful!\r\n".to_string();
        control_stream.write_all(success.as_bytes()).await.unwrap();
        ...
        loop {
            ...
            // Server task to convert incoming message to uppercase!
        ...
        control_stream.write_all(failure.as_bytes()).await.unwrap();
...
async fn main() {
    ...
    // Autenticate the server. See documentation of native-tls for more information.
    ...
    loop {
        ...
        // Spawn a new task to handle the communication.
        tokio::spawn(async move {
            // Accept the TLS communication.
            match tls_acceptor.accept(socket).await{
                ...
                Err(e) => {
                    println!("Server: TLS connection failed with client from {}!",  remote_addr);
                    log::error!("Server: TLS connection failed with client from {}!",  remote_addr);
                    log::error!("Server: {}", e);
                }
            }
